...
using the two-sided kolmogorov-smirnov test
check if extremes recon and obs time series
...
def getFiles(data):
    ...
    df = pd.DataFrame(columns = ['tg', 'lon', 'lat', 'reanalysis', 'pval'])
    ...
    for ii in range(0, len(tg_list)): 
        ...
        new_df = pd.DataFrame([tg, longitude, latitude, data,pval]).T
        new_df.columns = ['tg', 'lon', 'lat', 'reanalysis','pval']

        df = pd.concat([df, new_df], axis = 0)
        # print(df)
    
    os.chdir(outPath)
    saveName = data+"TwoSidedKolmogorovTest.csv"
    df.to_csv(saveName)
...
def getKTest(surgeMerged):
    ...
    if (surgeMerged.empty):
        ...
        p = ks_2samp(surgeMerged['surge'], \
                        surgeMerged['surge_reconsturcted'])[1]
        print(p)
